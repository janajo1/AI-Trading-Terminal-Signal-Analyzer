...
async function startServer() {
  ...
  // --- API ROUTE 1: Detailed Market Analysis ---
...
4. التوصية المقترحة مع الأسباب التقنية.`;
      ...
      // Smart fallback response if API key is not yet set or fails
      return res.json({
        success: true,
        data: {
          ...
          reasons: [
            "تأكيد شمعة ابتلاعية صاعدة على الإطار الزمني المSelected",
            "اختراق متوسط الحركة SMA50 بفلتر زخم قوي",
  ...
  // --- API ROUTE 2: Automatic AI Signal Generator (Entry, Stop Loss, TP1, TP2, TP3) ---
...
7. نسبة المخاطرة مقابل العائد Risk/Reward Ratio (مثال 1:3.2).
8. نسبة الثقة Confidence Score (مثال 89).
9. الأسباب والاستراتيجية المتبعة شرح باللغة العربية.`;
      ...
      // Algorithmic smart signal fallback calculation if offline / API key pending
      ...
      const entryPrice = cp;
      const stopLoss = Number((cp - factor * delta * 1.2).toFixed(4));
      const takeProfit1 = Number((cp + factor * delta * 1.5).toFixed(4));
      const takeProfit2 = Number((cp + factor * delta * 2.8).toFixed(4));
      const takeProfit3 = Number((cp + factor * delta * 4.2).toFixed(4));
  ...
  // --- API ROUTE 3: AI Custom Strategy Builder ---
